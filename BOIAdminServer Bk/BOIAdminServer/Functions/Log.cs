...
namespace GetOnline.Functions
{
    class Log
    {
        ...
        private static DevComponents.DotNetBar.RadialMenuItem radial_online;
        ...
        public static void updateGrid(DataTable result)
        {
            ...
            try
            {
                if (result != Log.grid_online.DataSource && result.Rows.Count > 0)
                {

                    Log.grid_online.DataSource = result;


                    Log.radial_online.Text = "Total ON:"+result.Rows.Count;
                }
            }
            catch { }
        }
